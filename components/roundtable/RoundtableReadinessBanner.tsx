...
const ctaPrimary = cn(
  ...
  "mt-3 inline-flex rounded-xl bg-cinnabar-600 text-card transition-[transform,box-shadow] duration-150 hover:bg-cinnabar-700 active:scale-[0.99]"
);

export function RoundtableReadinessBanner({ readiness }: { readiness: RoundtableReadiness }) {
  if (readiness.kind === "loading") {
    return (
      <div className="mb-6 rounded-xl bg-card px-4 py-3 text-sm text-ink-600 ring-border">
        正在确认你的登入与执笔是否就绪…
      </div>
    );
  }
  ...
  if (readiness.kind === "dev_local") {
    return (
      <div className="mb-6 rounded-xl bg-card px-4 py-3 text-sm text-ink-700 divider-l">
  ...
  if (readiness.kind === "no_backend") {
    return (
      <div className="mb-6 rounded-xl bg-destructive/5 px-4 py-3 text-sm text-ink-800 ring-destructive">
  ...
  if (readiness.kind === "need_login") {
    return (
      <div className="mb-6 rounded-xl bg-card px-4 py-3 text-sm text-ink-800 ring-destructive">
